...
function unlinkAssetsIOS(files, projectConfig) {
  ...
  if (!plist) {
    _logger.default.error('Could not locate "Info.plist" file. Check if your project has "INFOPLIST_FILE" set properly');

    return;
  }

  if (!project.pbxGroupByName('Resources')) {
    _logger.default.error('Group "Resources" does not exist in your Xcode project. There is nothing to unlink.');

    return;
  }

  const removeResourceFiles = (f = []) => (f || []).map(asset => project.removeResourceFile(_path().default.relative(projectConfig.sourceDir, asset), {
