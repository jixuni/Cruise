...
var _PackageManager = _interopRequireDefault(require("../PackageManager"));
 ...
 */
function createProjectFromTemplate(destPath, newProjectName, template, destinationRoot) {
...
function createFromRemoteTemplate(template, destPath, newProjectName, destinationRoot) {
  let installPackage;
  let templateName;
  ...
  _logger.default.info(`Fetching template ${installPackage}...`);

  const packageManager = new _PackageManager.default({
  ...
  try {
    ...
    try {
      ...
      _logger.default.warn(`Failed to clean up template temp files in node_modules/${templateName}. ` + 'This is not a critical error, you can work on your app.');
    }
  }
}
...
function installTemplateDependencies(templatePath, destinationRoot) {
  ...
  _logger.default.info('Adding dependencies for the project...');

  if (!_fs().default.existsSync(dependenciesJsonPath)) {
    _logger.default.info('No additional dependencies.');
  ...
  new _PackageManager.default({
    projectDir: destinationRoot
  }).install(dependenciesToInstall);

  _logger.default.info("Linking native dependencies into the project's build files...");
...
function installTemplateDevDependencies(templatePath, destinationRoot) {
  ...
  _logger.default.info('Adding develop dependencies for the project...');

  if (!_fs().default.existsSync(devDependenciesJsonPath)) {
    _logger.default.info('No additional develop dependencies.');
  ...
  new _PackageManager.default({
    projectDir: destinationRoot
  }).installDev(dependenciesToInstall);
}